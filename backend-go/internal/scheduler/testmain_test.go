...
// exactly the honest-boot discipline of the transport and webhooks lanes:
...
func TestMain(m *testing.M) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "scheduler test bootstrap: %v\n", err)
		os.Exit(1)
	}
	pg = cluster
	code := m.Run()
...
// every test truncates the org-rooted graph (everything CASCADEs from orgs;
// audit_events is the one org-rooted table without an FK) after itself —
// tests never leak rows into each other.
func testPool(t *testing.T) *pgxpool.Pool {
	t.Helper()
	cfg, err := pgxpool.ParseConfig(pg.DSN(pgtest.SharedDBName))
	if err != nil {
		t.Fatalf("parse pool config: %v", err)
	}
	// The four jobs each hold an advisory-lock connection for their whole tick
	// while the store's queries need further connections - size the test pool
	// so a concurrent four-job tick cannot exhaust it (the production pool is
	// sized by infra config; this mirrors that headroom).
	cfg.MaxConns = 16
	pool, err := pgxpool.NewWithConfig(context.Background(), cfg)
	if err != nil {
		t.Fatalf("connect scheduler test pool: %v", err)
	}
	...
	truncate() // start clean too — sibling packages on the same cluster may share the db
	t.Cleanup(truncate)
	return pool
}
