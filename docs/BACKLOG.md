...
|----|---------|------|-------|--------|------------------------------|
| F1 | Receivables core — Invoice→Receivable split, lifecycle states, aging, write-off ownership (H1) | 1 | #1 | **in-progress** (agent A) | Pure transition fns; all legal/illegal transitions tested |
| F2 | Payments core — dual-path intake (C2B + STK), idempotency unique keys (C5, R9, K1) | 1 | #2 | **in-progress** (agent B) | Duplicate callback returns same payment; tripwire event |
| F3 | Reconciliation — Match re-pointed to Payment; multi-invoice representable (C1, R5) | 1 | #3 | **in-progress** (agent B) | 3-invoices-1-payment scenario test passes |
| F4 | Adjustments — Refund + RefundAllocation (C2, R6), CreditNote + applications (C3, R7), CustomerCreditBalance (C4) | 1 | #4 | **in-progress** (agent C) | Ceiling rejections tested; overpayment→balance flow |
| F5 | Allocation engine — strategy chain FIFO/explicit/pro-rata on Money.allocate (H3, R1, R2) | 2 | #5 | pending | Cross-module R1/R2 suite; reversal flow |
| F6 | Event catalog as typed code + outbox contract (envelope, versioning) | 2 | #6 | pending | All 27 events typed; replay deterministic |
| F7 | Late fee accrual + PaymentPlan schedule engine (H4, H5) | 2 | #7 | pending | Accrual caps tested; installment generator |
...
| F10 | Consent registry (DPA 2019) + WhatsApp opt-in + eTIMS numbering hooks (K2–K4) | 2 | #10 | pending | No dunning without grant; number format reserved |
...
- Agent A → `feat/receivables-core` → PR for #1
- Agent B → `feat/payments-reconciliation` → PR for #2 + #3
- Agent C → `feat/adjustments-core` → PR for #4
