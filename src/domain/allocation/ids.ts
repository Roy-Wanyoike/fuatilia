/**
 * Allocation lane id helpers.
 *
 * The domain core is pure: no I/O, no RNG, no Date.now(). Ids are therefore
 * either supplied by the caller (adapters mint UUIDs) or derived
 * deterministically from a seed string so that replaying the same logical
 * command yields the same id — the foundation of idempotent replay
 * (same inputs → same allocations with same sequenceNo, issue #5).
 *
 * Mirrors the wave-1 payments lane helper; lanes cannot import each other,
 * so the tiny FNV-1a routine is duplicated by design.
 */
