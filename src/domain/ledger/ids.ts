/**
 * Ledger-lane id helpers (mirrors the payments lane — lanes import only
 * `../shared` + their own files, so the tiny helper is duplicated by design).
 *
 * The domain core is pure: no I/O, no RNG, no Date.now(). Ids are either
 * supplied by the caller (adapters mint UUIDs) or derived deterministically
 * from a seed string, so replaying the same logical command yields the same
 * id — a bonus for idempotent replay (e.g. the daily reconciliation job id is
 * a pure function of org + currency + run date).
 */
