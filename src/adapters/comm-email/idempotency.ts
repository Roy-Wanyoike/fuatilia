/**
 * Wire-layer idempotency for the email lane (issue #127, R9 discipline for
 * comms — the mirror of `src/adapters/comm-sms/idempotency.ts`): the crash
 * window between "the relay ACCEPTED the send" and "the worker persisted the
 * Message-ID" must never re-charge the provider call. A durable store
 ...
 *     (EMAIL_IN_FLIGHT — the worker replays later, R9);
 *   - a follower after the winner recorded sees the REAL result and replays
 *     it unchanged (same Message-ID — the same-outcome guarantee);
...
export interface EmailDispatchStore {
   ...
   */
  claim(key: string): { won: boolean; recorded: EmailWireResult | null };
  /** The winner records the real result after the wire call resolves. */
  record(key: string, result: EmailWireResult): void;
}

interface Entry {
  readonly result: EmailWireResult;
  readonly reservedAt: number;
  readonly placeholder: boolean;
}

/** In-memory store (tests + fakes). `now` is injectable for lease tests. */
export const inMemoryEmailDispatchStore = (options: { readonly now?: () => number; readonly leaseMs?: number } = {}): EmailDispatchStore => {
 ...
 */
export const emailDispatchKey = (req: EmailWireRequest, clientRef: string): string => {
  const parts = clientRef.split('#');
  const messageId = parts[0] ?? '';
  const attemptNo = Number(parts[1] ?? 0);
  if (messageId.trim() === '' || !Number.isInteger(attemptNo) || attemptNo < 1) {
    throw new DomainError('EMAIL_IDEMPOTENCY_KEY_INVALID', `clientRef must be "<messageId>#<attemptNo>", got "${clientRef}"`);
  }
  return `email:${req.to}:${messageId}:${attemptNo}`;
};
 ...
 */
export const withWireIdempotency = (transport: EmailTransport, store: EmailDispatchStore): EmailTransport => ({
  name: transport.name,
  async dispatch(req: EmailWireRequest): Promise<EmailWireResult> {
    if (req.clientRef === undefined || req.clientRef.trim() === '') {
      throw new DomainError('EMAIL_IDEMPOTENCY_KEY_REQUIRED', 'a dispatch without a clientRef cannot be made idempotent — refuse, never guess');
    }
    const key = emailDispatchKey(req, req.clientRef);
    const probe = store.claim(key);
    if (!probe.won) {
      const recorded = probe.recorded;
      if (recorded === null) {
        throw new DomainError('EMAIL_IDEMPOTENCY_STORE_INVALID', 'claim lost but no recorded result — store is not atomic');
      }
      if (!recorded.ok && recorded.failureReason === IN_FLIGHT) {
        throw new DomainError('EMAIL_IN_FLIGHT', `dispatch ${key} is in flight — replay after it resolves (R9)`);
      }
      return recorded;
    }
